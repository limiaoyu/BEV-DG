...
def validate(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            # preds_3d = model_3d(data_batch,preds_2d) if model_3d else None
            feats_2d = model_2d(data_batch)
            feats_3d = model_3d(data_batch) if model_3d else None

            last_number = 0
            batch_bev_feats = []
            for i in range(data_batch['img'].shape[0]):
                indices_pt = data_batch['img_indices'][i]
                num_pt = indices_pt.shape[0]
                cur_number = last_number + num_pt
                xyz_pt = data_batch['x'][0][last_number:cur_number, :3]
                feats_2d_pt = feats_2d[last_number:cur_number, :]
                feats_3d_pt = feats_3d[last_number:cur_number, :]
                # BEV fusion
                cat_feats, pool_3d, unq_inv, unq_cnt = BEV_fusion(xyz_pt, feats_2d_pt,
                                                                            feats_3d_pt)
            ...
            preds_2d = model_2d(data_batch, feats_2d, batch_bev_feats)
            preds_3d = model_3d(data_batch, feats_3d, batch_bev_feats) if model_3d else None
